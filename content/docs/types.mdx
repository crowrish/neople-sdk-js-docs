...
**최신 버전: 0.3.0** - 향상된 타입 안전성과 모든 API 엔드포인트에 대한 완전한 타입 정의를 제공합니다.
...
```bash
yarn add -D neople-openapi-types
```

### 주요 개선사항 (0.3.0)

- **업데이트된 API 스키마**: 최신 네오플 API 사양 반영
- **정확한 타입 매핑**: 모든 요청/응답에 대한 정확한 TypeScript 타입
- **게임별 특화 타입**: 던전앤파이터와 사이퍼즈 각각의 고유 데이터 구조
- **런타임 타입 검증**: 타입 가드 함수 및 스키마 검증 지원
