...
const client = new NeopleDFClient(apiKey, {
  httpAdapter: 'got',
  gotInstance: got.extend({
    isStream: true // 스트리밍 모드 활성화
  })
});

// 대용량 데이터 스트리밍 다운로드
async function downloadLargeData(url: string, filePath: string) {
...
class RedisCache extends Map {
  ...
  async set(key: string, value: any) {
    await redis.setex(key, 3600, JSON.stringify(value)); // 1시간 TTL
    return this;
  }
...
const client = new NeopleDFClient(apiKey, {
  ...
  gotInstance: got.extend({
    https: {
      certificateAuthority: fs.readFileSync('path/to/ca.pem'),
      key: fs.readFileSync('path/to/client-key.pem'),
      certificate: fs.readFileSync('path/to/client-cert.pem'),
